...
local function parse_snippets(content)
	...
	while true do
		...
		if not title_start then
			break
		end

		local title_end = content:find("\n", title_start + 3)
		if not title_end then
			break
		end
		...
		if #gallery.images > 0 then
			table.insert(galleries, gallery)
		end
...
local function download_phosphor_icons(output_dir)
	...
	for _, variant in ipairs(variants) do
		...
		if css_resp.ok then
			...
			css = css:gsub(
				'url%("%./' .. variant.pattern .. '%.woff2"%)',
				'url("/fonts/' .. variant.font_name .. '.woff2")'
			)
			css = css:gsub(
				'url%("%./' .. variant.pattern .. '%.woff"%)',
				'url("/fonts/' .. variant.font_name .. '.woff")'
			)
			css =
				css:gsub('url%("%./' .. variant.pattern .. '%.ttf"%)', 'url("/fonts/' .. variant.font_name .. '.ttf")')
			css = css:gsub(
				'url%("%./' .. variant.pattern .. '%.svg#.-"%)',
				'url("/fonts/' .. variant.font_name .. '.svg")'
			)
			all_css = all_css .. css .. "\n"
			...
			for _, img in ipairs(images) do
				...
				if rel_path then
					...
					if dir_path then
						...
						rs.image_convert(
							img_path,
							ctx.output_dir .. "/snippets/" .. dir_path .. "/" .. img.stem .. ".webp",
							{ quality = 85 }
						)
					end
				end
